...
全站索引在服务端按月切分。客户端不做任何自动补全：冷启动只取 `search-index/meta.json`（数十 KB）与 `timeline-recent.json`，时间线立即可用，历史内容一律按需下载。

入口是全站时间线副标题的「按日期」，弹出年 → 月 → 日的日期树。月份与条数来自 `meta.json`，因此**尚未下载的月份同样会完整列出**，点击即开始下载；点未下载月份下的某一天，会先下载该月再跳转。年行与月行右侧都有下载按钮，下载中显示实时百分比，已下载的长按可删除。

分片落盘于 `filesDir/global_index/`，命中时先比对 `meta.json` 里的 `hash`（SHA-256 前 16 位），一致则直接读盘不发请求。上次下载过的月份在冷启动时自动合入，无需联网。

该目录**刻意不放在 `index_cache/` 下**：设置页的「清理缓存」会递归删除 `index_cache/`，而历史月份是用户主动下载的内容，不应被顺手清掉。它的清理入口只在日期面板里。

`GlobalIndexMeta.total` 始终是全站真实总条数，与本地下载了多少无关，所以副标题上的「共 599,520 条」任何时候都准确；有搜索、账号筛选或日期筛选时才切换为当前结果数。

几个容易踩的点：

- 分片可能有十几 MB，`HttpTimeout` 未设置 `requestTimeoutMillis`——该项限制整个请求的总时长，慢速网络下会在传输正常进行时误杀请求。真正的卡死由 `socketTimeoutMillis` 覆盖。
- 下载进度每 64 KB 回调一次，一个分片近两百次，仅在百分比整数变化时才写入 StateFlow，否则会引发大量无谓重组。
- 合并分片需要对累计数十万条重排，该操作在 `Dispatchers.Default` 上执行；整年下载只在全部月份就绪后重建一次，而不是每合入一个月重排一遍。
- 删除月份时必须同步清掉内存中的 `monthPosts`，只删文件的话推文仍会留在时间线上。
